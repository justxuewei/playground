(function () {
  ...
  const stages = [
    {
      ...
      note: "Read the 2D tiling kernel in execution order: block index, thread tile, shared memory, register caches, K loop, writeback."
    },
    {
      title: "Block setup",
    ...
    {
      title: "Tile work",
      note: "Each K chunk loads A[128x8] and B[8x128] into shared memory."
    },
    {
      title: "Register tile",
      note: "Each dot step forms an 8x8 outer product from regM[8] and regN[8]."
    },
    {
      title: "Next K chunk",
    ...
    {
      title: "Why faster",
      note: "Kernel 5 spends more shared memory and registers to get much more arithmetic per thread."
    }
  ];
  ...
  function drawCodeLine(x, y) {
    const lines = [
      "sgemm2DBlocktiling<BM=128, BN=128, BK=8, TM=8, TN=8>",
      "threadCol = threadIdx.x % 16; threadRow = threadIdx.x / 16;",
      "As[128x8] and Bs[8x128] cache one K chunk;",
      "threadResults[8x8] += regM[8] outer regN[8];",
      "A += BK; B += BK * N;",
      "kernel5 trades more shared/register storage for more FMAs;",
  ...
  function drawCodeStage(progress) {
    ...
    roundRect(codeX - 18, codeY - 34, 680, 500, 8, "#ffffff", "#d8dde8");
    ctx.fillStyle = "#1d2433";
    ...
    const steps = [
      ...
      ["Shared memory", "As has 128x8 floats, Bs has 8x128 floats."],
      ["Registers", "Each thread keeps 64 C accumulators."],
      ["Load chunk", "256 threads cooperatively fill As and Bs."],
      ["Register cache", "regM[8] and regN[8] feed an outer product."],
      ["Next chunk", "A and B pointers advance by 8 along K."],
      ["Write C", "The 64 registers become an 8x8 C patch."]
    ];
  ...
  function drawTileWorkStage(progress) {
    ...
    drawCallout(394, 100, [
      ...
      "",
      "256 threads load the tiles in strides."
    ]);
  }
  ...
  function drawRegisterTileStage(progress) {
    ...
    drawCallout(742, 430, [
      "Kernel 5's main step:",
      "",
      "Load 8 values from As into regM.",
      "Load 8 values from Bs into regN.",
      "",
      "The thread computes 8 x 8 FMAs",
      "for its register tile."
    ]);
  }
  ...
  function drawMatrixStage(progress) {
    ...
    drawCallout(812, 126, [
      "Kernel 5 compared with kernel 4:",
      "",
      "Kernel 4:",
      "  one thread -> 8x1 C values",
      "",
      "Kernel 5:",
      "  one thread -> 8x8 C values"
    ]);
  }

  function drawWhyFasterStage(progress) {
    drawComparisonPanel(70, 118, "Kernel 4", {
      tile: "C tile: 64x64",
      shared: "shared: 1024 floats",
      thread: "thread result: 8x1",
      loads: "per dotIdx: 8 As + 1 Bs",
      fmas: "8 FMAs",
      fill: "#f4f7fb",
      stroke: "#9aa7bb"
    });
    drawComparisonPanel(440, 118, "Kernel 5", {
      tile: "C tile: 128x128",
      shared: "shared: 2048 floats",
      thread: "thread result: 8x8",
      loads: "per dotIdx: 8 As + 8 Bs",
      fmas: "64 FMAs",
      fill: "#e8f7ef",
      stroke: "#118a4b"
    });
    ...
    drawMetricRow(835, 188, "Shared memory", "2x", "#2563eb");
    drawMetricRow(835, 242, "C results/thread", "8x", "#6d28d9");
    drawMetricRow(835, 296, "FMAs/dotIdx/thread", "8x", "#118a4b");

    ctx.fillStyle = "#5b6475";
    ctx.font = "13px ui-sans-serif, system-ui";
    ctx.fillText("Kernel 5 is faster when the extra", 835, 368);
    ctx.fillText("register/shared-memory cost is paid back", 835, 389);
    ctx.fillText("by higher arithmetic reuse.", 835, 410);

    drawFormula(292, 560, "Kernel 5: 16 shared-memory values -> 64 FMAs through an 8x8 register tile");
  }
  ...
  function drawSharedTiles(x, y, progress) {
    ...
    drawThinMatrix("As shared 128x8", x, y, 170, 170, {
      fill: "#dbeafe",
      stroke: "#2563eb",
      label: "128 rows x 8 cols"
    });
  ...
  function drawSharedHighlights(x, y, dot) {
    ...
    ctx.fillRect(x + dot * cell, y + cell, cell, cell);
    ctx.strokeStyle = "#2563eb";
    ctx.lineWidth = 2;
    ctx.strokeRect(x + dot * cell, y + cell, cell, cell);

    const bx = x + 250;
    ctx.fillStyle = "rgba(180, 83, 9, 0.24)";
    ctx.fillRect(bx, y + dot * cell, cell, cell);
    ctx.strokeStyle = "#b45309";
    ctx.lineWidth = 2;
    ctx.strokeRect(bx, y + dot * cell, cell, cell);
  }
