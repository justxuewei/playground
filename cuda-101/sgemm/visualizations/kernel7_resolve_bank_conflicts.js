(function () {
  ...
  const stages = [
    {
      ...
      note: "Read kernel 7 in execution order: block tile, vector loads, transposed As, remapped Bs, register compute, float4 C writeback."
    },
    {
      title: "Block setup",
      note: "The block shape stays the same as kernels 5 and 6: 256 threads compute one 128x128 C tile."
    },
    {
      title: "Tile work",
    ...
    {
      title: "Bank layout",
      note: "Bs is linearized so each thread can read regN[8] with fewer shared-memory bank conflicts."
    },
    {
      title: "Next K chunk",
      note: "The K loop advances by BK=8 and repeats the same remapped shared-memory load."
    },
    {
      title: "Matrix grid",
    ...
    {
      title: "Why faster",
      note: "Kernel 7 keeps kernel 6's bytes and math, but makes shared-memory reads less serialized."
    }
  ];
  ...
  function drawCodeLine(x, y) {
    const lines = [
      "sgemmResolveBankConflicts<BM=128, BN=128, BK=8, TM=8, TN=8>",
      "blockIdx=(0,0) owns C rows 0..127 and cols 0..127;",
      "A and B use float4 global loads;",
      "Bs[((innerColB % 2) * 4 + innerRowB * 8 + lane) * 16 + innerColB / 2];",
      "regN[i] = Bs[(dotIdx * 8 + i) * 16 + threadCol];",
      "A += BK; B += BK * N;",
      "gridDim repeats 128x128 C tiles;",
      "kernel7 remaps Bs to reduce bank conflicts;"
    ];
  ...
  function drawCodeStage(progress) {
    ...
    const codeLines = [
      ...
      "Bs[((innerColB % 2) * 4 + innerRowB * 8 + 0) * 16 + innerColB / 2] = bLoad.x;",
      "Bs[((innerColB % 2) * 4 + innerRowB * 8 + 1) * 16 + innerColB / 2] = bLoad.y;",
      "Bs[((innerColB % 2) * 4 + innerRowB * 8 + 2) * 16 + innerColB / 2] = bLoad.z;",
      "Bs[((innerColB % 2) * 4 + innerRowB * 8 + 3) * 16 + innerColB / 2] = bLoad.w;",
      "__syncthreads();",
    ...
    ctx.fillText("Kernel 7 execution order", codeX, codeY - 10);
    ...
    const steps = [
      ...
      ["Remap Bs", "The four values are stored into a bank-aware layout."],
      ["Sync", "All shared-memory writes are visible to the block."],
      ["Read regN", "regN reads the remapped Bs address pattern."],
  ...
  function drawBlockStage(progress) {
    ...
    drawCallout(62, 124, [
      ...
      "C cols 0..127",
      "",
      "Same block geometry as kernel 6.",
      "Only Bs shared-memory layout changes."
    ]);
    drawCallout(790, 448, [
  ...
  function drawTileWorkStage(progress) {
    ...
    drawCallout(394, 100, [
      ...
      "",
      "Kernel 7 keeps vectorized global",
      "loads from kernel 6."
    ]);
  }

  function drawBankLayoutStage(progress) {
    drawBankComparison(70, 118, "Kernel 6 Bs", false, progress);
    drawBankComparison(460, 118, "Kernel 7 Bs", true, progress);
    drawArrow(405, 296, 445, 296, "#118a4b", ease(progress));
    drawCallout(850, 132, [
      "The B values are the same.",
      "",
      "Only their shared-memory addresses",
      "change before the compute loop.",
      "",
      "regN reads:",
      "Bs[(dotIdx * 8 + i) * 16 + threadCol]"
    ]);
    drawFormula(232, 582, "Bs remap: ((innerColB % 2) * 4 + innerRowB * 8 + lane) * 16 + innerColB / 2");
  }
  ...
  function drawNextKStage(progress) {
    ...
    drawCallout(690, 125, [
      ...
      "",
      "Every chunk uses the same Bs",
      "bank-conflict-aware remap."
    ]);
  }
  ...
  function drawMatrixStage(progress) {
    ...
    drawCallout(812, 126, [
      "Kernel 7 compared with kernel 6:",
      "",
      "Same 128x128 block tile",
      "Same 8x8 thread result tile",
      "",
      "Different Bs layout in shared memory"
    ]);
  }

  function drawWhyFasterStage(progress) {
    drawComparisonPanel(70, 118, "Kernel 6", {
      tile: "C tile: 128x128",
      shared: "Bs layout: natural 8x128",
      thread: "thread result: 8x8",
      loads: "global load/store: float4",
      fmas: "risk: bank conflicts on Bs",
      fill: "#f4f7fb",
      stroke: "#9aa7bb"
    });
    drawComparisonPanel(440, 118, "Kernel 7", {
      tile: "C tile: 128x128",
      shared: "Bs layout: remapped",
      thread: "thread result: 8x8",
      loads: "global load/store: float4",
      fmas: "goal: fewer bank conflicts",
      fill: "#e8f7ef",
      stroke: "#118a4b"
    });
    ...
    drawMetricRow(835, 296, "Bs bank pattern", "better", "#118a4b");
    ctx.fillStyle = "#5b6475";
    ctx.font = "13px ui-sans-serif, system-ui";
    ctx.fillText("Kernel 7 is faster when shared-memory", 835, 368);
    ctx.fillText("loads of Bs serialize less, so the same", 835, 389);
    ctx.fillText("FMAs are fed with fewer stalls.", 835, 410);
    drawFormula(220, 560, "Kernel 7: same bytes and same 8x8 math, but remapped Bs reduces shared-memory conflicts");
  }
  ...
  function drawSharedTiles(x, y, progress) {
    ...
    drawBankedMatrix("Bs shared, remapped", x + 250, y, 170, 170);
    ctx.globalAlpha = 1;
  }

  function drawBankComparison(x, y, title, remapped, progress) {
    roundRect(x - 18, y - 42, 330, 382, 8, "#ffffff", remapped ? "#118a4b" : "#9aa7bb");
    ctx.fillStyle = "#1d2433";
    ctx.font = "700 16px ui-sans-serif, system-ui";
    ctx.fillText(title, x, y - 16);
    ctx.fillStyle = "#5b6475";
    ctx.font = "12px ui-sans-serif, system-ui";
    ctx.fillText(remapped ? "reads spread across banks" : "natural layout can collide", x, y + 320);

    const cell = 18;
    for (let row = 0; row < 8; row += 1) {
      for (let col = 0; col < 16; col += 1) {
        const bank = remapped ? (row * 5 + col * 3) % 8 : col % 4;
        const hot = bank === Math.floor(progress * 8) % 8;
        ctx.fillStyle = hot ? "rgba(180, 83, 9, 0.55)" : bankColor(bank);
        ctx.fillRect(x + col * cell, y + row * 28, cell - 2, 22);
      }
    }
    ctx.strokeStyle = remapped ? "#118a4b" : "#b45309";
    ctx.lineWidth = 2;
    ctx.strokeRect(x, y, cell * 16 - 2, 8 * 28 - 6);
  }
  ...
  function drawBankedMatrix(title, x, y, w, h) {
    ...
    for (let row = 0; row < rows; row += 1) {
      for (let col = 0; col < cols; col += 1) {
        ctx.fillStyle = bankColor((row * 5 + col * 3) % 8);
    ...
    ctx.fillStyle = "#5b6475";
    ctx.font = "12px ui-sans-serif, system-ui";
    ctx.fillText("same 1024 floats, different addresses", x, y + h + 22);
  }
  ...
  function drawChunkTimeline(x, y, progress) {
    ...
    drawFormula(x + 30, y + 172, "load float4 -> remap Bs -> sync -> register compute -> next chunk");
  }
